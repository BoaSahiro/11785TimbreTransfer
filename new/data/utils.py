...
def spsi_eff(magD, y_out, fftsize, hop_length):
    ...
    for i in range(50):
        S = magD * np.exp(1j * p)
        x = librosa.istft(
            S, hop_length, win_length=fftsize, center=True
        )  # Griffin Lim, assumes hann window; librosa only does one iteration?
...
def magnitude2waveform(magD, config, phase, length=None):
    ...
    if phase is None:
        ...
        audio = spsi_eff(
            magD, y_out, fftsize=config["fft_size"], hop_length=config["hop_length"]
        )
        ...
        audio = librosa.istft(
            stft_matrix, hop_length=config["hop_length"], length=length
        )
    ed = time.time()
...
def spectrum2magnitude(spec, config):
    ...
    melfb = librosa.filters.mel(
        22050, config["fft_size"], n_mels=config["n_mels"]
...
def get_spectrogram(data, config, win_len=None):
    ...
    stft_matrix = librosa.stft(
        data, n_fft=config["n_fft"], hop_length=config["hop_length"], win_length=win_len
    )
    mag_D = np.abs(stft_matrix)
    pwr = mag_D**2

    mel_basis = librosa.filters.mel(
        sr=config["sr"], n_fft=config["n_fft"], n_mels=config["n_mels"]
    )
...
def get_cqt(data, config):
    cqt_matrix = librosa.cqt(
        data, hop_length=config["hop_length"]
    )
    return cqt_matrix
...
def get_cepstrogram(spec, config, win_len=None):
    ...
    mel_ceps_coef = scipy.fftpack.dct(
        spec, axis=0, type=config["dct_type"], norm=config["norm"]
    )
...
def get_spectral_envelope(mel_spec, config):
    MFCC = scipy.fftpack.dct(
        mel_spec, axis=0, type=config["dct_type"], norm=config["norm"]
    )
    hei, wid = MFCC.shape
    MFCC[15:, :] = 0.0
...
def plot_figure(img_name, mel_spec, config):
    ...
    hei, wid = mel_spec.shape  # 256, 302

    plt.figure(
        1, figsize=(7 * (wid / 302), 7 / 302 * 256)
    )  # the resolution of y-axis goes up with hei
    plt.clf()

    plt.subplot(1, 1, 1)
    librosa.display.specshow(
        mel_spec, y_axis="mel", x_axis="time", hop_length=config["hop_length"]
    )
    plt.colorbar()

    row_anchor = 1
    # if cqt_list:
    #     # plot cqt                                  vvv i-th row           vvv i-th col
    #     plt.subplot(
    #         1,
    #         1,
    #         row_anchor + (i + 1),
    #     )
    #     librosa.display.specshow(
    #         cqt_list[i], x_axis="time", hop_length=config["hop_length"]
    #     )
    #     plt.colorbar()
    #     plt.title("corresponding cqt")
    #     row_anchor += 1
