...
def get_rid_of_prefix(some_string):
    split_list = some_string.split('.')
    res = ""
    for segment in split_list[1:]:
        res += segment
        res += '.'
    return res[:-1]
...
def predict(spectrogram=None, model_dir=None, params=None, device=torch.device('cuda'), fast_sampling=False):
  ...
  if not model_dir in models:
    ...
    try:
      ...
      new_state_dict = OrderedDict([(get_rid_of_prefix(k), v) for k, v in old_state_dict.items()])
...
def main(args):
  ...
  spectrogram_name, ext = os.path.splitext(os.path.basename(args.spectrogram_path))
  torchaudio.save(spectrogram_name + '.wav', audio.cpu(), sample_rate=sr)
